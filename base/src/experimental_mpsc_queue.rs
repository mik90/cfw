...
impl<T> ExperimentalMpscQueue<T> {
    pub fn new(capacity: usize) -> Self {
        assert!(
            capacity >= 2,
            "ExperimentalMpscQueue requires capacity >= 2"
        );
        ...
        loop {
            ...
            if seq != h + 1 {
                ...
                if seq == h + self.capacity {
                    let _ = self.head.compare_exchange_weak(h, h + 1, Relaxed, Relaxed);
            ...
            match self.head.compare_exchange_weak(h, h + 1, Acquire, Relaxed) {
                Ok(_) => {
                    ...
                    unsafe {
                        std::ptr::swap((*slot.value.get()).as_mut_ptr(), return_value.as_mut_ptr());
                    }
...
mod experimental_tests {
    ...
    #[test]
    #[cfg_attr(miri, ignore = "Not worth testing in miri yet")]
